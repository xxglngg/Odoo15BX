...
class AccountGeneralLedgerReport(models.AbstractModel):
    ...
    def _get_templates(self):
        ...
        templates['line_template'] = 'custom_accounting.line_template_general_ledger_report'
        templates['main_template'] = 'custom_accounting.main_template_with_filter_input_accounts'
        return templates
    ...
    def _get_columns_name(self, options):
        ...
        if self.user_has_groups('base.group_multi_currency'):
            columns_names.insert(7, {'name': _('Currency'), 'class': 'number'})
    ...
    def _get_query_amls_select_clause(self):
        return '''
            ...
            move.no_invoice                                 AS no_invoice,
            move.no_efaktur                                 AS no_efaktur,
            analytic_account.name                           AS analytic_account_id,
    ...
    def _get_aml_line(self, options, account, aml, cumulated_balance):
        ...
        columns = [
            ...
            {'name': aml['no_invoice'], 'class': 'o_account_report_line_ellipsis'},
            {'name': aml['no_efaktur'], 'class': 'o_account_report_line_ellipsis'},
            {'name': aml['partner_name'], 'class': 'o_account_report_line_ellipsis'},
            {'name': aml['analytic_account_id'], 'class': 'o_account_report_line_ellipsis'},
        ...
        if self.user_has_groups('base.group_multi_currency'):
            if (aml['currency_id'] and aml['currency_id'] != account.company_id.currency_id.id) or account.currency_id:
                ...
                currency = False
            columns.insert(6, {'name': currency and aml['amount_currency'] and self.format_value(aml['amount_currency'], currency=currency, blank_if_zero=True) or '', 'class': 'number'})
        return {
            'id': aml['id'],
    ...
    def _get_load_more_line(self, options, account, offset, remaining, progress):
        return {
            ...
            'colspan': self.user_has_groups('base.group_multi_currency') and 10,
            'columns': [{}],
        }
    ...
    def _get_total_line(self, options, debit, credit, balance):
        return {
            ...
            'colspan': self.user_has_groups('base.group_multi_currency') and 8 or 6,
        }
    ...
    def _get_tax_declaration_lines(self, options, journal_type, taxes_results):
        lines = [{
            ...
            'columns': [{'name': ''}],
            'colspan': self.user_has_groups('base.group_multi_currency') and 10 or 9,
            'level': 1,
            'unfoldable': False,
            'unfolded': False,
        }, {
