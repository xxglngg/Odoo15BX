...
class ReportAccountAgedPartnerLedger(models.AbstractModel):
    _name = "account.aged.partner.ledger"
    _description = "Aged Partner Ledger Balances"
    ...
    balance = fields.Monetary(string='Initial Balance')
    ...
    def _get_templates(self):
        # OVERRIDE
        templates = super(ReportAccountAgedPartnerLedger, self)._get_templates()
    ...
    def _get_column_details(self, options):
        columns = [
            ...
            self._field_column('balance', name=_('Initial Balance'), sortable=True),
            self._field_column('debit', sortable=True),
            self._field_column('credit', sortable=True),
            self._field_column('amount_currency', name=_('Current'), sortable=True),
    ...
    def _get_hierarchy_details(self, options):
        return [
            self._hierarchy_level('partner_id', foldable=True, namespan=len(self._get_column_details(options)) - 10),
    ...
    def _format_total_line(self, res, value_dict, options):
        res['name'] = _('Total')
        res['colspan'] = len(self._get_column_details(options)) - 10
        res['columns'] = res['columns'][res['colspan']-1:]


class ReportAccountPartnerLedgerAR(models.Model):
    _name = "account.partner.ledger.ar"
    _description = "Partner Ledger Aged Receivable"
    _inherit = "account.aged.partner.ledger"
    _auto = False

    def _get_options(self, previous_options=None):
        # OVERRIDE
        options = super(ReportAccountPartnerLedgerAR, self)._get_options(previous_options=previous_options)
        options['filter_account_type'] = 'receivable'
        return options

    @api.model
    def _get_report_name(self):
        return _("Partner Ledger AR")

    @api.model
    def _get_templates(self):
        # OVERRIDE
        templates = super(ReportAccountPartnerLedgerAR, self)._get_templates()
        templates['line_template'] = 'custom_accounting.line_template_partner_ledger_ar_report'
        return templates