...
class SaleOrder(models.Model):
    ...
    def _sh_unreseve_qty(self):
        ...
        if self.sudo().mapped('picking_ids'):
            for picking in self.sudo().mapped('picking_ids'):
        ...
        if self.company_id.cancel_delivery and self._check_stock_installed():
            if self.company_id.operation_type == 'cancel':
                if self.sudo().mapped('picking_ids'):
                    ...
                    # cancel packages
                    if self.sudo().mapped('picking_ids').sudo().mapped('move_line_ids').mapped('result_package_id'):
                        packages = self.sudo().mapped('picking_ids').sudo().mapped('move_line_ids').mapped('result_package_id')
                        if packages:
                            packages.unpack()
                        
                    if self.sudo().mapped('picking_ids').sudo().mapped('package_level_ids_details'):
                        self.sudo().mapped('picking_ids').sudo().mapped('package_level_ids_details').sudo().unlink()


                    self._sh_unreseve_qty()
                    if self.sudo().mapped('picking_ids').sudo().mapped(
                            'move_ids_without_package'):
                ...
                if self.sudo().mapped('picking_ids'):
                    ...
                    picking_ids = self.sudo().mapped('picking_ids')
                    if self.sudo().mapped('picking_ids').sudo().mapped(
                            'move_ids_without_package'):
                        self.sudo().mapped('picking_ids').sudo().mapped(
                            'move_ids_without_package').sudo().write(
                    ...
                    if reconcile_lines:
                        ...
                        if payments:
                            ...
                            if payment_ids.sudo().mapped('move_id').mapped(
                                ...
                                payment_lines = payment_ids.sudo().mapped(
                                    'move_id').mapped('line_ids')
                                reconcile_ids = payment_lines.sudo().mapped(
                                    'id')
